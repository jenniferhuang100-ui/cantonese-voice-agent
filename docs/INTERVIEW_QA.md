...
- **Real incident from this repo**: "I migrated providers, redeployed, and Railway crash-looped — `OpenAIError: Missing credentials`. Root cause: the new SDK's client constructor raises immediately on a missing key, at *import time*, before Flask starts — unlike the old SDK, which only failed on first call. That silently broke my own `MOCK_MODE` design: 'no key → degrade to scripted demo' never got a chance to run because the process died before reaching that check. Fix was two-layered: a placeholder key so construction never crashes, restoring the intended degrade-to-mock behavior, plus actually setting the real key in Railway's dashboard. Verified by simulating Railway's exact condition locally — zero env vars, no `.env` file — and confirming it now boots clean instead of crash-looping. The lesson I'd generalize: a provider migration can silently break a *fallback path* you didn't touch, because the new SDK's failure mode differs from the old one's. Test the failure path, not just the happy path, after any dependency swap."
