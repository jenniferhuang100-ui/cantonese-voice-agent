...
2. ~~**OpenAI client construction crashed the whole process when `DEEPSEEK_API_KEY` was unset.**~~ ✅ Fixed 2026-07-17. The `openai` SDK's client constructor raises immediately on a missing key (checked at import time), unlike the old `anthropic` SDK, which only failed on first use. That meant `MOCK_MODE`'s "no key → fall back to scripted demo" design was silently broken by the provider migration — Railway crash-looped on boot instead of degrading to mock mode. Fix: `client = OpenAI(api_key=DEEPSEEK_API_KEY or "unset-falls-back-to-mock-mode", ...)` — construction never crashes, and `MOCK_MODE` (which guards every code path that actually calls `client`) does its job as designed. Verified locally with zero key/env present: boots clean, `/health` responds, `/chat` correctly serves the mock FSM.
